...
async fn main() -> std::io::Result<()> {
    ...
    for symbol in opts.symbols.split(',') {
        handle_symbol(&symbol, &from, &to).await;
    }
    Ok(())
}

async fn handle_symbol(symbol: &str, from: &DateTime<Utc>, to: &DateTime<Utc>) -> Option<()> {
    let closes = fetch_closing_data(&symbol, &from, &to).await.ok()?;
    if !closes.is_empty() {
        // min/max of the period. unwrap() because those are Option types
        let period_max: f64 = MaxPrice.calculate(&closes).await?;
        let period_min: f64 = MinPrice.calculate(&closes).await?;
        let last_price = closes.last()?;
        let (_, pct_change) = PriceDifference.calculate(&closes).await?;
        let sma = WindowedSMA { window_size: 30 }.calculate(&closes).await?;
